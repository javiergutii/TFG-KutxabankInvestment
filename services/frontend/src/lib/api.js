...
async function getGraphToken(msalInstance, accounts) {
  const { graphRequest } = await import("../authConfig");
  try {
    const response = await msalInstance.acquireTokenSilent({
      ...graphRequest,
      account: accounts[0],
...
export async function inspectForm(msalInstance, accounts, url) {
...
export async function launchJob(msalInstance, accounts, payload) {
  const headers = await getAuthHeaders(msalInstance, accounts);
  const graphToken = await getGraphToken(msalInstance, accounts);
  const res = await fetch(`${API_BASE}/api/jobs`, {
    method: "POST",
    headers,
    body: JSON.stringify({ ...payload, graph_token: graphToken }),
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
