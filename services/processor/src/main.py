...
# Asegurar que los imports usan el config del PROCESSOR, no del extractor
_PROCESSOR_SRC = os.path.dirname(os.path.abspath(__file__))
if _PROCESSOR_SRC not in sys.path:
    sys.path.insert(0, _PROCESSOR_SRC)

# Eliminar el path del extractor si está presente para evitar conflictos
_EXTRACTOR_SRC = '/app/src'
if _EXTRACTOR_SRC in sys.path:
    sys.path.remove(_EXTRACTOR_SRC)
